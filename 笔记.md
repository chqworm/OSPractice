...
引导扇区代码从硬盘载入内核代码。因为我们的os代码不只512字节，我们需要从硬盘读取内核代码来运行内核。我们不必直接控制处理磁盘，我们只需要调用bios的例程（一段代码），就像我们在屏幕上打印字符。要读取硬盘数据，需将ah设置为0x02，并且其他寄存器设置所要读取的柱面，磁头和扇区号，然后调用 int 0x13。
